            ...
            <div class="gx-wrapper">
                <div class="animated slideInUpTiny animation-duration-3">
                    <div class="page-heading d-sm-flex justify-content-sm-between align-items-sm-center">
                        <h2 class="title mb-3 mb-sm-0">{{ Auth::guard('state')->user()->state }} State Executive Lists</h2>
                        <nav class="mb-0 breadcrumb">
                            <a href="/state" class="breadcrumb-item">Dashboard</a>
                            
                            <span class="active breadcrumb-item">Executive Lists</span>
                        </nav>
                    </div>
                                ...
                                <div class="gx-card-body">
                                    <div class="table-responsive">
                                        <table class="table table-users default-table table-hover">
                                            ...
                                            <td>{{ date('d M Y', strtotime($executive->created_at)) }}</td>
                                                <td>
                                                  <div>
                                                    <a href="/state/exestate/{{ $executive->id }}" class="btn btn-info"><i class="fa fa-eye"></i>View</a>
                                                </div>
                                                </td>
                                
                                                <td><a href="/state/exestate/{{ $executive->id }}/edit" class="btn btn-primary">Edit</a></td>

                                        
                                                <td class="text-center">
                                                <form method="post" action="/state/exestate/{{ $executive->id }}">
                                                 @method('DELETE')
                                                 @csrf
