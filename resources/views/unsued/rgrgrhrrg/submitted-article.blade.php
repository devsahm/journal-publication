            ...
            <div class="gx-wrapper">
                <div class="animated slideInUpTiny animation-duration-3">
                    <div class="page-heading d-sm-flex justify-content-sm-between align-items-sm-center">
                        <h2 class="title mb-3 mb-sm-0">Submitted Article</h2>
                        <nav class="mb-0 breadcrumb">
                            <a href="/admin" class="breadcrumb-item">Dashboard</a>
                            
                            <span class="active breadcrumb-item">Submitted Articles</span>
                        </nav>
                    </div>
                                        ...
                                        <table class="table table-users default-table table-hover">
                                          ...
                                          <!--     <th>Reference Number</th> -->
                                            ...
                                            <tr>
                                                ...
                                                <td>{{ $article->email }}</td>
                                              <!--   <td> NAOIP{{ now()->year }}0{{ $article->id }}</td> -->
                                                <td>{{ date('d M Y', strtotime($article->created_at)) }}</td>
                                                <td>
                                                  <div>
                                                    <a href="/article/{{ $article->id }}" class="btn btn-info" style="margin-left: 20px" ><i class="fa fa-eye-slash"></i>View</a>
                                                </div>
                                                </td>
                                                <td>
                                                  ...
                                                  <label class="">
                                                    <input type="checkbox" class="" name="confirmed" onChange="this.form.submit()" {{ $article->confirmed? 'checked' : '' }}> 
                                                    <span class=""></span>
                                                    <span class="" >{{ $article->confirmed? 'Confirmed' : 'Confirm Article' }}</span>
                                                   </label> 

                                             <!--    <div class="form-control custom-checkbox custom-control-inline">
                                                <input type="checkbox"  name="confirmed" id="customcheckboxInline" class="custom-control-input" onChange="this.form.submit()" {{ $article->confirmed? 'checked' : '' }}>
                                                <label class="custom-control-label">{{ $article->confirmed? 'Confirmed' : 'Confirm Article' }}</label>
                                            </div> -->

                                                </form>
                                                </td>
                                                <td><a href="/admin/send-email/{{ $article->id }}" class="btn btn-primary">Compose Mail</a></td>
